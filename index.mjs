...
addon.setEnvironmentVariables({
  ...
  LANGUAGE: {
    ...
    description: 'The language of wake words.',
...
function updateKeywords() {
    ...
    for (const fileName of fileNames) {
      ...
      if (fileExtension === '.zip') {
        try {
          ...
          const ppnEntry = entries.find((entry) => entry.entryName.endsWith('.ppn'))
          if (ppnEntry) {
  ...
  if (keywords.size === 0) {
    addon.errors.push('At least one wake word must be added to the data volume.')
  }
